...
class GPS2RTC
{
 public:
  ...
  // This began life as a constructor but it couldn't handle the HardwareSerial.
  {
	...
	rtc_time_set = false;
	rtc_waiting_for_tcr_reset = false;
	rtc_compensation_set = false;
  ...
  // Handle one record sent from the GPS receiver.
  {
	  ...
	  if (state == looking_for_serial_data && l_gps_time != 0 && l_fix_age != TinyGPS::GPS_INVALID_AGE && l_fix_age <= 2000) {
		...
		state = waiting_for_tcr_reset;				    // This enables the wait for the RTC reset to take effect
		rtc_waiting_for_tcr_reset = true;
		rtc_reset_count = sample_time_secs + 1;
	  }
  ...
  // Handle an interrupt generated by the rising edge of the 1PPS signal from the GPS receiver.
  {
	int l_new_time;
	  ...
	  if (handle_interrupts) {
		switch (state) {
		case waiting_for_tcr_reset:
		  rtc_reset_count--;
		  if (rtc_reset_count == 0) {
			state = waiting_for_1pps;
			rtc_waiting_for_tcr_reset = false;
		  }
		  break;
  ...
  static bool rtc_waiting_for_tcr_reset;
  static bool rtc_compensation_set;
